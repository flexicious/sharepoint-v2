(function(window)
{
    ...
    PagerControl.prototype.destroy=function(){
        this.destroyButtons([PagerControl.ACTION_FIRST_PAGE,
            ...
            PagerControl.ACTION_ADD_ITEM,
            PagerControl.ACTION_EDIT_ITEM,
            PagerControl.ACTION_DELETE_ITEM
        ]);
        var pageDropDown=this.getPageDropdown();
        if(pageDropDown){
        ...
        this.grid.removeEventListener("ShowEditDeleteButtons", this.showEditDeleteButtons);
        this.grid.removeEventListener("HideEditDeleteButtons", this.hideEditDeleteButtons);
        this.grid.removeEventListener("EnableEditDeleteButtons", this.enableEditDeleteButtons);
        this.grid.removeEventListener("DisableEditDeleteButtons", this.disableEditDeleteButtons);
    };
    ...
    PagerControl.prototype.initialize=function(){
        ...
        if(this.level.getNestDepth()==1){
            html+= (
            "<button id='"+gridId+"editItem' class='pagerDiv iconCell pager-control-button' style='display : none'><span class='imageButtonEditItem' ><img tabindex='-1' id='"+gridId+"editItemImg' tabindex='0' src='images/edit_ico.png' alt='Edit Item' title='Edit Item'/>Edit</span></button>"+
            "<button id='"+gridId+"deleteItem' class='pagerDiv iconCell pager-control-button' style='display : none'><span class='imageButtonDeleteItem'> <img tabindex='-1' id='"+gridId+"deleteItemImg' tabindex='0' src='images/cancel.png' alt='Delete Item' title='Delete Item'/>Delete</span></button>"+
            "<span id='"+gridId+"addItem' class='pagerDiv  iconCell'><img tabindex='0' src='images/addItem.png' class='imageButtonAddItem' alt='Add Item' title='Add Item'></span>")+
            "<span  class='pagerDiv lineSep'>&nbsp;</span>"+
        ...
        this.grid.addEventListener(this,"ShowEditDeleteButtons", this.showEditDeleteButtons);
        this.grid.addEventListener(this,"HideEditDeleteButtons", this.hideEditDeleteButtons);
        this.grid.addEventListener(this,"EnableEditDeleteButtons", this.enableEditDeleteButtons);
        this.grid.addEventListener(this,"DisableEditDeleteButtons", this.disableEditDeleteButtons);

        this.grid.dispatchEvent(new flexiciousNmsp.BaseEvent("PagerEventsAdded"));

        flexiciousNmsp.UIComponent.prototype.initialize.apply(this );

    };

    PagerControl.prototype.showHideEditDeleteButtons = function(show){
        var id=this.grid.id||this.grid.domElement.id;
        id+="_";
        var but =  document.getElementById(id+"editItem");
        if(but){
            but.style.display = show ? "" : "none";
        }
        but = document.getElementById(id+"deleteItem");
        if(but){
            but.style.display = show ? "" : "none";
        }
    };

    PagerControl.prototype.enableDisableEditDeleteButtons = function(enabled){
        var id=this.grid.id||this.grid.domElement.id;
        id+="_";
        var but = document.getElementById(id+"editItem");
        if(but){
        ...
        but = document.getElementById(id+"deleteItem");
        img = document.getElementById(id+"deleteItemImg");
        if(but){
            if(enabled)
                but.removeAttribute("disabled");
            else
    ...
    PagerControl.prototype.showEditDeleteButtons = function (event) {
        this.showHideEditDeleteButtons(true);
    };

    PagerControl.prototype.hideEditDeleteButtons = function (event) {
        this.showHideEditDeleteButtons(false);
    };

    PagerControl.prototype.enableEditDeleteButtons = function (event) {
         this.enableDisableEditDeleteButtons(true);
    };

    PagerControl.prototype.disableEditDeleteButtons = function (event) {
        this.enableDisableEditDeleteButtons(false);
    };
    ...
    PagerControl.ACTION_ADD_ITEM="addItem";
    PagerControl.ACTION_EDIT_ITEM="editItem";
    PagerControl.ACTION_DELETE_ITEM="deleteItem";
    ...
    PagerControl.prototype.processAction=function(code){
        ...
        }else if(code==PagerControl.ACTION_ADD_ITEM){
            this.grid.dispatchEvent(new flexiciousNmsp.FlexDataGridEvent("AddItemClick"));
        }else if(code==PagerControl.ACTION_EDIT_ITEM){
            var id=this.grid.id||this.grid.domElement.id;
            id+="_";
            var but = document.getElementById(id+"editItem");
            if(!but.hasAttribute("disabled"))
                this.grid.dispatchEvent(new flexiciousNmsp.FlexDataGridEvent("EditItemClick"));
        }else if(code==PagerControl.ACTION_DELETE_ITEM){
            var id=this.grid.id||this.grid.domElement.id;
            id+="_";
            var but = document.getElementById(id+"deleteItem");
            if(!but.hasAttribute("disabled"))
             this.grid.dispatchEvent(new flexiciousNmsp.FlexDataGridEvent("DeleteItemClick"));
        }
    ...
    GridController.pagerControl = flexiciousNmsp.PagerControl = PagerControl;
}(window));