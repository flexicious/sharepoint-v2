(function(window){
	...
	{
		this._pageSize=val;
		var pageSizeDropDown = this.getPageSizeDropDown()
		if(pageSizeDropDown)
			pageSizeDropDown.value = val;
	};
	...
	PagerControl.prototype.getPageSizeDropDown=function(){
		return uiUtil.adapter.findElementWithClassName(this.domElement,"pageSizeDropDown");
	};
	...
	PagerControl.prototype.initialize=function(){
		...
		var html="<span class='pagerTable' style='float: left;height:"+this.getHeight()+"px'>" +
			...
			(this.level.enablePaging?"<span  class='pagerDiv lineSep'>&nbsp;</span>":"")+
			(/*this.grid.enableFlexiblePageSize*/ !0 && this.level.enablePaging ? "<span>Page Size <select class='pageSizeDropDown'></select></span>" : "" )+
			(this.level.enablePaging?"<span  class='pagerDiv lineSep'>&nbsp;</span>":"")+
			"</span>";
		...
		var pageSizeDropDown = this.getPageSizeDropDown();
		if(this.level.enablePaging && /*this.grid.enableFlexiblePageSize*/ !0){
			pageSizeDropDown.pagerControl=this;
			var step = this.grid.pagerControlPageSizeStep ? this.grid.pagerControlPageSizeStep : 5;
			var minSize = this.grid.pagerControlPageSizeMin ? this.grid.pagerControlPageSizeMin : 5;
			var maxSize = this.grid.pagerControlPageSizeMax ? this.grid.pagerControlPageSizeMax : 100;
			for(var i = minSize; i <= maxSize; i+=step){
				var opt = document.createElement('option');
				opt.setAttribute('value', i);
				opt.innerHTML = i;
				pageSizeDropDown.appendChild(opt);
			}
			pageSizeDropDown.value = this.getPageSize();
			uiUtil.addDomEventListener(this,pageSizeDropDown,"change",onPageSizeDropDownChange)
		}
	...
	PagerControl.prototype.refresh=function(){
		...
		if(pageInfo)
			...
			+". "+flxConstants.PGR_PAGE+" "+(this.getPageIndex()+1)+" "+flxConstants.PGR_OF+" "+this.getPageCount() ;//+ (":"+this.level.getNestDepth()+":"+this.getPageIndex());
	...
	var onPageSizeDropDownChange=function(event){
		var target = event.currentTarget || event.srcElement;
		var pageSize = target.value;
		var pagerControl= target.pagerControl;
		pagerControl.setPageSize(parseInt(pageSize));
		pagerControl.setPageIndex(0);
		pagerControl.refresh();
		pagerControl.level.pageSize = parseInt(pageSize);
		if(pagerControl.doDispatchEvents)
			pagerControl.dispatchEvent(new flexiciousNmsp.ExtendedFilterPageSortChangeEvent(flexiciousNmsp.ExtendedFilterPageSortChangeEvent.PAGE_CHANGE));
		pagerControl.grid.refreshCells();
		var e = new flexiciousNmsp.BaseEvent('pageSizeChanged');
		e.level = pagerControl.level;
		e.grid = pagerControl.grid;
		pagerControl.grid.dispatchEvent(e);
	};
	...
	flexiciousNmsp.FlexDataGridColumnLevel.static_FlexDataGridPager= new flexiciousNmsp.ClassFactory(flexiciousNmsp.PagerControl,undefined); // force all level use this pager control
}(window));
